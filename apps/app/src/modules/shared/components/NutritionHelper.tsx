...
import {useNutrition} from "@modules/shared/queries/useNutrition.tsx";
...
export const NutritionHelper = () => {
  ...
  const productFiltered: Product[] = useMemo(() => {
    const items: Product[] = data?.productList ?? [];
  ...
  const {data: dataN, isLoading: isLoadingN, refetch: refetchN, error: errorN} = useNutrition({
    enabled: false,
    filters: {
      text: "",
      ids: selectedProducts?.length ? selectedProducts.map(m => m.id) : undefined,
      g_index_min: activeFilters.g_index_min,
      g_index_max: activeFilters.g_index_max,
      g_load_min: activeFilters.g_load_min,
      g_load_max: activeFilters.g_load_max,
  ...
  useEffect(() => {
    if (selectedProducts.length && !isLoadingN) {
      refetchN()
    }
  }, [activeFilters, selectedProducts, isLoadingN]);
  ...
  return (
    <>
      ...
      <div className="sticky bg-background top-0 p-4 pb-8 mx-[-1rem] z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4">
          ...
          <div className="relative pr-12">
            ...
            />
            <Button disabled={isLoadingN} onClick={handleSearch}
                    className="cursor-pointer absolute w-10 top-0 right-0">
      ...
      <div className="selected-products sticky z-[11] top-[84px] mb-4">
        <div className="bg-background rounded-xl p-2 -m-2 mt-0 shadow-sm">
          {selectedProducts.length > 0 ? <div className="flex flex-wrap gap-2">
            ...
            <span
              className="text-muted-foreground inline-block min-h-[30px] bg-primary/10 px-2 py-1 rounded-md text-sm border border-primary/20">Ничего не выбрано</span>
          )}

          <div className="relative mt-4 max-h-40 min-h-10 overflow-y-auto">
            {isLoadingN ?
              <div className="absolute inset-0 flex items-center justify-center">
                <Spinner className="h-6 w-6 text-primary"/>
              </div>
            ...
            {errorN ?
              <div className="flex-1 error"><PrettyJSON json={errorN}></PrettyJSON></div>
              : <div className="whitespace-pre-wrap" dangerouslySetInnerHTML={{__html: dataN?.recommendation}}/>}
          </div>
        </div>
      </div>
